...
const headerLeftAlignmentContainer = document.querySelector('.header-left-alignment-container');
...
const deleteProfileBtn = getButton('Profil Löschen',['header-btn','neg','hidden']);
const visibleInEditMode = [saveBtn,discardBtn,deleteProfileBtn,displayNameInput,descriptionInput,changePasswordContainer];
const invisibleInEditMode = [editBtn,logoutBtn,displayNameContainer,descriptionContainer];
...
function quitEditMode() {
    ...
    let popup = showPopup('Änderungen verwerfen?','Bist du sicher, dass du alle Änderunegn verwerfen möchtest?',{
        discardChanges: {
...
async function loadData() {
    ...
    if(!username)
    {
        let popup = showPopup('User nicht gefunden','Das Profil des Nutzers wurde nicht gefunden.',{
            leaveBtn: {
                text: 'Zurück'
            }
        },false);
    ...
    if(!res.success) {
        endLoadingAnimation(loadingAnimation);
        let popup = showPopup('User nicht gefunden',`Das Profil des Nutzers '${username}' wurde nicht gefunden.`,{
            leaveBtn: {
                text: 'zurück'
            }
        },false);
    ...
    if(username === api.getLoggedInUser())
    {
        ...
        logoutBtn.addEventListener('click', (e) => {
            ...
            window.location = './profile.html?msg=efolgreichh ausgeloggt&msg-duration=4000&msg-type=correct&username='+username;
        });
        ...
        deleteProfileBtn.addEventListener('click', (e) => {
            let popup = showPopup('Profil löschen','Bist du dir sicher, dass du dein Profil entgültig löschen möchtest.',{
                cancel: {
            ...
            popup.buttons.delete.addEventListener('click', async (e) => {
                ...
                let deleteSuccessPopup = showPopup('Profil gelöscht','Du hast dein Profil erfolgreich gelöscht',{
                    back : {
                        text: 'Zur Startseite'
                    }
                });
                deleteSuccessPopup.buttons.back.addEventListener('click', (e) => {
                    window.location = '../index.html';
                },false);
            });
...
async function saveChanges() {
    ...
    if(password !== repeatedPassword) {
        showPopup('Fehler', 'Die eingegebenen Passwörter stimmen nicht überein');
        return false;
    }
    ...
    let res = await api.updateUser(username, password, displayName, description);
